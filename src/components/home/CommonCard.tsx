...
const MainCard: React.FC = () => {
  return (
    <div className="grid sm:flex gap-6 justify-center sm:mt-16 relative p-6 sm:p-4">
      ...
      {learn_practice_mentorship.map((item, index) => (
        <div className="w-full sm:w-3/12" key={index}>
          <div className="relative imgHov transition-all cursor-pointer transform hover:translate-y-[-15px] duration-300">
            <h1
              className={`w-full ${item.textColor} text-center absolute bottom-4 sm:bottom-2 font-bold text-[16px] lg:text-[24px]
